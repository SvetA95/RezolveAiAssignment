...
environment variables for this reason. Ideally a dedicated test customer/service would be used
too, rather than one shared with other teams or candidates, so a failed run can't be blamed on
someone else's test data.

**Failure behavior:** a red build should mean something actionable. The two `0`/`-1` cases
that fail (see `docs/findings-ticket.md`, BUG-1) would get tagged, either `@Disabled` with a
reason or a JUnit tag excluded from the default run, so the pipeline stays meaningfully
