...
| A6 | Two updates in quick succession | POST `2`, then immediately POST `7` | Final value is `7`, confirming updates apply in order |
...
| B2 | **[Automated]** Zero | Rejected, since a batch of 0 servers can't update anything | Accepted (202) and stored as `0`. This is a bug, see BUG-1 |
...
| C3 | **[Automated]** Decimal (`1.5`) | Rejected, since a batch size has to be a whole number of servers | Yes, 400 |
| C4 | **[Automated]** Leading/trailing whitespace | Rejected, or trimmed and accepted; either is fine as long as it's consistent | Both rejected with 400, consistently |
...
| D3 | **[Automated]** GET/POST against an unknown service | 404 | Yes, with a different (generic) error body than D2 for what should be the same kind of error |
...
| E2 | **[Automated]** Scope isolation | Set the property for one customer's `fas:live1`; check another customer's `fas:live1` | Untouched. This property should be scoped per customer and service, not shared globally | Yes, confirmed, and now an automated test (`doesNotLeakAcrossCustomers`) |
...
- **Read-after-write timing (E1).** The spec documents `202 Accepted`, which is normally a
  hint that something happens asynchronously. Even though it turned out instant here, I didn't
  want the automated check to quietly depend on that.
